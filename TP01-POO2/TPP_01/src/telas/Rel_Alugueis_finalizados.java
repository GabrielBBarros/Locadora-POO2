...
public final class Rel_Alugueis_finalizados extends JDialog {
	
	private static Controlador controlador;
	private static Rel_Alugueis_finalizados relAluFinInst;
	
	private final JPanel contentPanel = new JPanel();
	private JTable tabela;
	private String [][] dados;
	private String [] titulos = {"Codigo A.", "Cliente", "Corretor", "Imovel", "D. Aluguel", "D. Devolucao","D. Pagamento","Total A.", "Valor A."};
	private int linha;
	private int coluna;
	...
	public static void main(String[] args) {
		try {
			Rel_Alugueis_finalizados dialog = Rel_Alugueis_finalizados.getInstancia(Controlador.getInstancia());
	...
	public static Rel_Alugueis_finalizados getInstancia(Controlador control) {
		
		if(relAluFinInst == null) {
			relAluFinInst = new Rel_Alugueis_finalizados(control);
		}
		return relAluFinInst;
	}
	
	
	private Rel_Alugueis_finalizados(Controlador control) {
		controlador = control.getInstancia();
		setBounds(100, 100, 900, 600);
		setIconImage(Toolkit.getDefaultToolkit().getImage("src/arquivos/bandeira_brasil.jpg"));
		setTitle("Alugueis finalizados");
	...
	public void setTamDadosalugueis() {
		this.dados = new String[controlador.alugueisFinalizados().size()][9];
	}
	
	public void setTabela() {
		ArrayList<Aluguel> alugueis = controlador.alugueisFinalizados();

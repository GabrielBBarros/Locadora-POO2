...
public final class Cad_Aluguel extends JDialog {
	...
	private Cad_Aluguel(Controlador control) {
		...
		//Data devolucao - 4
		JLabel devolucaoLabel = new JLabel("Data de Devolucao");
		...
		//Data devolucao - 4.2
		JLabel mesLavel = new JLabel("Mes");
		...
		botaoCadastrar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					if(control.cadastroAluguel(clienteTXT.getText().trim().toUpperCase(), corretorTXT.getText().trim().toUpperCase(), imovelTXT.getText().trim().toUpperCase(), hoje, dev, pg)) {
						control.salvarArquivos();
						erroSucesso.setText("<html><center>Aluguel cadastrado com Sucesso!!!</center></html>");
						clearTxt();
					}
					else {
						erroSucesso.setText("<html><center>Aluguel nao cadastrado. Erro ocorreu!!!</center></html>");
					}
				} catch (Exception e2) {
					erroSucesso.setText("<html><center>Aluguel nao cadastrado. Erro exception!!!</center></html>");
					e2.printStackTrace();
				}
			}
		});

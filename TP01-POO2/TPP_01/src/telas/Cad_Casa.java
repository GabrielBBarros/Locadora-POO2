...
public final class Cad_Casa extends JDialog{

	private static Cad_Casa cadCasaInst;
	...
	public static void main(String[] args) {
		try {
			Cad_Casa casa = Cad_Casa.getInstancia(control.getInstancia());
			casa.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
	...
	public static Cad_Casa getInstancia(Controlador control) {
		if(cadCasaInst == null) {
			cadCasaInst = new Cad_Casa(control);
		}
		return cadCasaInst;
	}

	public Cad_Casa(Controlador control) {
		...
		botaoCadastrar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					if(control.cadastroCasaResidencial(endereco, data, fl, in)) {
						control.salvarImovel();
						clearTXT();
						erroSucesso.setText("<html><center>Casa cadastrada com Sucesso!!!</center></html>");
					}
					else {
						erroSucesso.setText("<html><center>Casa nao cadastrada. Erro ocorreu!!!</center></html>");
					}
				} catch (Exception e2) {
					erroSucesso.setText("<html><center>Casa nao cadastrada. Erro ocorreu!!!</center></html>");
					e2.printStackTrace();
